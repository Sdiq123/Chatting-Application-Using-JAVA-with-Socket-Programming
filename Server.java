...
import java.net.*; // both the server socket class and the server client class is available in this module
import java.io.*; // Provides both the data input and the data ouput streams to both send and recieve the messages


public class Server /*extends JFrame*/ implements ActionListener { // simple extend wont make the JFrame static  hence we need to make an object of it 
    ...
    Server() {
        ...
        ImageIcon i4 = new ImageIcon(ClassLoader.getSystemResource("icons/resized2.png")); // creating an image icon class which will help me in getting the image 
        ...
        JLabel name = new JLabel ("SADIQ  "); // Main Function Of JLabel is used to write on the frame 
        ...
        f.setLocation(200, 50); //by default frame opens from the left corner
    ...
    public static void main(String args[]) {
        ...
        try{                // While running a Server we experience many errors hence to overcome it putting it in try catch block
            ServerSocket skt = new ServerSocket(6001); // Creating a server Object with help of ServerSocket class and mapping it to a port number 
            while(true){
                Socket s = skt.accept();  // i have to make my server run infinitely accept all the images , accepting all the messages and storing it in socket s , what do you mean by infinitely send message once you send a message then again you send a message , if you want to send multiple times then it should be in infinite loop
                ...
                dout = new DataOutputStream(s.getOutputStream()); // if you need to send the message 
